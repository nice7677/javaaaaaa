...
public class Insert_Profile_Form extends Parent_Form {
	...
	public Insert_Profile_Form(FrontController control, Member account) {
		...
		cboBloodType.addActionListener(new ActionListener(){
			...
			public void actionPerformed(ActionEvent e) {
				// TODO Auto-generated method stub
				JComboBox ch = (JComboBox) e.getSource();
				account.setBloodT(ch.getSelectedItem().toString());
			}
		});
			//상대방 혈액형 선택 콤보박스 이벤트
		cboIdeal_BloodType.addActionListener(new ActionListener(){
			...
			public void actionPerformed(ActionEvent e) {
				// TODO Auto-generated method stub
				JComboBox ch = (JComboBox) e.getSource();
				account.setMyTypeB(ch.getSelectedItem().toString());
			}
		});
	...
	public Insert_Profile_Form(FrontController control, Member account, int mode) {
		super(550, 800, control);
		setTitle("InsertProfilePage");
		...
		txtAge.setText(account.getAge().toString());
		txtPhone.setText(account.getPhone().toString());
		if(account.getGender().equals("남자"))
			rdoMale.setSelected(true);
		else
			rdoFemale.setSelected(true);
		cboRegion.setSelectedItem(account.getRegion());
		cboIdeal_BloodType.setSelectedItem(account.getMyTypeB());
		cboBloodType.setSelectedItem(account.getBloodT());
		txtIntroduce.setText(account.getIntro());
		txtCharacter.setText(account.getMyType());
